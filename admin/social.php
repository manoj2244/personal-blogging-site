...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `social_media` WHERE `id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  $title4=mysqli_real_escape_string($db,$_POST['title4']);
    ...
    $sql="UPDATE `social_media` SET `facebook` = '$title', `youtube` = '$title1',`instagram`='$title3',`linkdin`='$title4' WHERE `social_media`.`id` = $news_id;";
  ...
  $sql="INSERT INTO `social_media` (`facebook`, `youtube`,`instagram`,`linkdin`) VALUES ('$title', '$title1','$title3','$title4')";
                ...
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">Edit social_media</h5>
...
?>
  ...
  if($insert==1){
    ...
    Your social_media has been inserted successfully
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($insert==0){
   echo" <div class='alert alert-danger alert-dismissible fade show' role='alert'>
    Your social_media is not added sucessfully!
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($update==1){
    echo "<div class='alert alert-primary alert-dismissible fade show' role='alert'>
    Your social_media has been updated successfully
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($update==0){
   echo" <div class='alert alert-danger alert-dismissible fade show' role='alert'>
    Your social_media is not updated sucessfully!
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
    ...
    <table class="table" id="myTable">
      <thead>
        <tr>
          ...
          <th width="30%" scope="col">Facebook</th>
          <th width="20%" scope="col">Youtube</th>
          <th width="20%" scope="col">Instagram</th>
          <th width="20%" scope="col">linkedin</th>
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this data!")) {
          console.log("yes");
          window.location = `../admin/social.php?delete=${sno}`;
