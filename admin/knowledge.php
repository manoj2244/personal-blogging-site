...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `knowledge` WHERE `id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    $sql="UPDATE `knowledge` SET `title` = '$title' WHERE `knowledge`.`id` = $news_id;";
  ...
  $sql="INSERT INTO `knowledge` (`title`) VALUES ('$title')";
                ...
                <div class="modal-dialog">
                  <div class="modal-content">
                    ...
                    <div class="form-group">
                      <label for="titles">knowledge</label>
                      <input type="text" class="form-control" id="title" name="title" value="">
                    </div>

                   
                    <button type="submit" class="btn btn-primary my-2">Add knowledge</button>
                  </form>
                    </div>
...
?>
  ...
  if($insert==1){
    ...
    Your knowledge has been inserted successfully
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($insert==0){
   echo" <div class='alert alert-danger alert-dismissible fade show' role='alert'>
    Your knowledge is not added sucessfully!
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($update==1){
    echo "<div class='alert alert-primary alert-dismissible fade show' role='alert'>
    Your knowledge has been updated successfully
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($update==0){
   echo" <div class='alert alert-danger alert-dismissible fade show' role='alert'>
    Your knowledge is not updated sucessfully!
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  ...
  ?>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
      <div class="form-group">
        <label for="title">knowledge_name</label>
    ...
    <table class="table" id="myTable">
      <thead>
        <tr>
          ...
          <th width="25%" scope="col">knowledge_Title</th>
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this knowledge!")) {
          console.log("yes");
          window.location = `../admin/knowledge.php?delete=${sno}`;
